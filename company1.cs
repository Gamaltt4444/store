...
{
    public partial class company1 : Form
    {
        ...
        public void LoadCustomer()
        {
            ...
            cm = new SqlCommand("SELECT nid,Nam , Mas , dec FROM Com", con);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtCName.Text == "")
                {
                    MessageBox.Show(" من فضلك ادخال اسم الشركة", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;

                }
                if (txtCPhone.Text == "")
                {
                    MessageBox.Show(" من فضلك ادخال نوع الشركة", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                ...
                if (MessageBox.Show("هل انت متاكد من حفظ البيانات ؟  ", "جاري الحفظ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("INSERT INTO Com (Nam,Mas,dec)VALUES(@date, @price,@dec)", con);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("هل انت متاكد من تعديل البيانات  ", "جاري التعديل", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("UPDATE Com SET Nam= @price,Mas=@Mas,dec=@cphone WHERE nid LIKE '" + lblCId.Text + "' ", con);
        ...
        private void dgvCustomer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM Com WHERE nid LIKE '" + dgvCustomer.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
