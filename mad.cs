...
namespace InventorySystem1._0
{
    public partial class mad : Form
    {
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT  * FROM Mad ", con);
        ...
        public void LoadProduct1()
        {
            int i = 0;
            dgvProduct.Rows.Clear();
            cm = new SqlCommand("SELECT  * FROM Mad WHERE  pname =@mad", con);
            cm.Parameters.AddWithValue("@mad", txtSearch.Text);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM Mad WHERE pid LIKE '" + dgvProduct.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtPName.Text == "")
                {
                    MessageBox.Show(" من فضلك ادخال اسم المندوب", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                ...
                if (MessageBox.Show(" هل انت متاكد انك تريد حفظ بيانات هذا المندوب؟", "جاري الحفظ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("INSERT INTO Mad(pname,pqty,phone,pdescription)VALUES(@pname, @pqty, @pprice, @pdescription)", con);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("هل انت متاكد انك تريد تعديل البيانات ؟", "جاري التعديل", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("UPDATE Mad SET pname = @pname, pqty=@pqty, phone=@pprice, pdescription=@pdescription WHERE pid LIKE '" + lblPid.Text + "' ", con);
                    cm.Parameters.AddWithValue("@pname", txtPName.Text);
                    cm.Parameters.AddWithValue("@pqty", txtPQty.Text);
                    cm.Parameters.AddWithValue("@pprice", txtPPrice.Text);
                    cm.Parameters.AddWithValue("@pdescription", txtPDes.Text);
