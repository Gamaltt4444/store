...
namespace InventorySystem1._0
{
    public partial class Employee : Form
    {
        ...
        public void LoadCustomer()
        {
            ...
            cm = new SqlCommand("SELECT nid, date ,Wnam,Work, price  FROM Employee", con);
        ...
        private void dgvCustomer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (colName == "Delete")
            {
                ...
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM Employee WHERE nid LIKE '" + dgvCustomer.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtCName.Text == "")
                {
                    MessageBox.Show(" من فضلك ادخال اسم الموظف", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;

                }
                if (txtCPhone.Text == "")
                {
                    MessageBox.Show(" من فضلك ادخال الوظيفة", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;

                }
                if (textBox1.Text == "")
                {
                    MessageBox.Show(" من فضلك ادخال المرتب ", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                ...
                if (MessageBox.Show("هل انت متاكد من حفظ البيانات ؟  ", "جاري الحفظ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("INSERT INTO Employee (date,Wnam,Work,price)VALUES(@date,@Wnam,@Work,@price)", con);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("هل انت متاكد من تعديل البيانات  ", "جاري التعديل", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("UPDATE Employee SET Wnam= @Wnam,Work=@Work, price= @price WHERE nid LIKE '" + lblCId.Text + "' ", con);
                    cm.Parameters.AddWithValue("@Wnam", txtCName.Text);
                    cm.Parameters.AddWithValue("@Work", txtCPhone.Text);
                    cm.Parameters.AddWithValue("@price", textBox1.Text);
                    con.Open();
